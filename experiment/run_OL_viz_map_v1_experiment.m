...
all_exps = 1:length(expi.settings.stim_struct);
early_exps = [10:45 154:171];
late_exps = [1:9 46:153];

% randomize order 
exp_order = [];
for ii = 1:expi.settings.num_reps

    exp_order = [exp_order early_exps(randperm(length(early_exps)))];
    
end

for ii = 1:expi.settings.num_reps

    exp_order = [exp_order late_exps(randperm(length(late_exps)))];
