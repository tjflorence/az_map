clear all
homedir = pwd;

pattern.x_num = 96; 	% There are 96 pixel around the display (12x8) 
pattern.y_num = 100; 		% two frames of Y, at 2 different spatial frequencies
pattern.num_panels = 48; 	% This is the number of unique Panel IDs required.
pattern.gs_val = 3; 	% This pattern will use 8 intensity levels
pattern.row_compression = 0;

loom_frames = make_off_loom();
Pats = ones(32, 96, pattern.x_num, pattern.y_num);

for k = 1:pattern.x_num
 
    for j = 1:pattern.y_num
    
        c_y_frame = loom_frames(:,:,j);
        Pats(:,:,k,j) = circshift(c_y_frame, [0 k-1]);
    
    end

end

pattern.Pats = Pats;
pattern.Panel_map = [12 8 4 11 7 3 10 6 2  9 5 1;...
                        24 20 16 23 19 15 22 18 14 21 17 13;...
                        36 32 28 35 31 27 34 30 26 33 29 25; ...
                        48 44 40 47 43 39 46 42 38 45 41 37];
                    
pattern.BitMapIndex = process_panel_map(pattern);
pattern.data = Make_pattern_vector(pattern);

directory_name = '/Users/florencet/Documents/matlab_root/az_map/patterns/007_loom/patterns';
str = [pwd '/patterns/Pattern_02_off_loom'];
save(str, 'pattern');

cd(homedir)
