function process_az_map(expdir)
...
for ii = 1:num_stim_types
    ...
    for jj = 1:length(c_type_exp) % jj for each rep of stim type
        ...
        if isfield(expr.c_trial, 'idata')
            ...
            summary_stim(ii).name = expr.settings.stim_struct(ii).name;
            summary_stim(ii).xpos_vec = expr.settings.stim_struct(ii).xpos_vec;
            summary_stim(ii).ypos_vec = expr.settings.stim_struct(ii).ypos_vec;
            summary_stim(ii).s_tstamp = expr.c_trial.bdata.timestamp(1:length(summary_stim(ii).xpos_vec));
            summary_stim(ii).pat_id = expr.settings.stim_struct(ii).pat_id;
